...
namespace GraphicsControls
{
    public partial class Stepper
    {
        ...
        void DrawCupertinoStepperSeparator(ICanvas canvas, RectangleF dirtyRect)
        {
            canvas.SaveState();

            canvas.StrokeSize = 1;
            canvas.StrokeColor = ColorHelper.GetGraphicsColor(Cupertino.Color.Label.Light.Quaternary, Cupertino.Color.Label.Dark.Quaternary);
        ...
        void DrawCupertinoStepperMinus(ICanvas canvas, RectangleF dirtyRect)
        {
            canvas.SaveState();
            ...
            canvas.FillColor = new GColor(Application.Current?.RequestedTheme == OSAppTheme.Light ? Cupertino.Color.Label.Light.Primary : Cupertino.Color.Label.Dark.Primary);

            canvas.FillPath(path);

            canvas.RestoreState();

            _minusRect = new RectangleF(tX, tY, CupertinoStepperHeight / 2, CupertinoStepperHeight / 2);
        }

        void DrawCupertinoStepperPlus(ICanvas canvas, RectangleF dirtyRect)
        {
            canvas.SaveState();
            ...
            canvas.FillColor = ColorHelper.GetGraphicsColor(Cupertino.Color.Label.Light.Primary, Cupertino.Color.Label.Dark.Primary);

            canvas.FillPath(path);

            canvas.RestoreState();

            _plusRect = new RectangleF(tX, tY, CupertinoStepperHeight / 2, CupertinoStepperHeight / 2);
        }
    }
}